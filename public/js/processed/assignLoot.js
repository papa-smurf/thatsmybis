...
$(document).ready(function () {
  ...
  $("select[name^=item][name$=\\[character_id\\]]:visible").addClass("selectpicker").selectpicker(); // If the current element has a value, show it and the next element that is hidden because it is empty

  function showNext(currentElement) {
    if ($(currentElement).val() != "") {
      $(currentElement).show();
      var nextElement = $(currentElement).closest(".row").next(".js-hide-empty");
      nextElement.show();
  ...
  }).change(); // Filter out characters based on the raid group they are in
  ...
  }).change(); // Toggles visibility on the Note inputs
  ...
  }).change(); // Toggles visibility on the Date inputs
  ...
  }).change(); // Sets all dates to the chosen date
...
function completeCsvImport(results) {
  ...
  console.log("Loading data into form..."); // Load the results into the form

  if (results.data.length > 0) {
    // Clear the form
    prepareForm(); // Load the parsed items into the form

    var inputRow = 0;

    for (var i = 0; i < results.data.length; i++) {
      var item = results.data[i]; // Skip over requests to disenchant an item

      var disenchantFlags = ['de', 'disenchant'];
      ...
      if (i >= maxItems) {
        ...
        console.log("Skipping row ".concat(i + 1, ": Disenchant ").concat(item.item ? item.item : item.item_id ? item.item_id : ''));
        skippedCount++;
        disenchantCount++;
        continue;
      } else {
    ...
    makeWowheadLinks(); // Update the fancy select for any select options we just chose.
  ...
  if (firstError) {
    statusMessages += "<li class=\"text-danger\">Error: ".concat(firstError.message, "</li>");
  }

  if (addedCount) {
    statusMessages += "<li class=\"text-success\">".concat(addedCount, " item").concat(addedCount > 1 ? 's' : '', " loaded into form</li>");
  }

  if (skippedCount) {
    statusMessages += "<li class=\"text-warning\">Skipped ".concat(skippedCount, " item").concat(skippedCount > 1 ? 's' : '', " ").concat(disenchantCount ? "(" + disenchantCount + " items disenchanted)" : "", " ").concat(overLimitCount ? "(" + overLimitCount + " items over the limit of " + maxItems + ")" : "", "</li>");
  }

  if (offspecCount) {
    statusMessages += "<li class=\"text-warning\">".concat(offspecCount, " item").concat(offspecCount > 1 ? 's' : '', " flagged as offspec</li>");
  }

  if (missingCharacterCount) {
    missingCharacters = missingCharacters.join(", ");
    statusMessages += "<li class=\"text-danger\">".concat(missingCharacterCount, " character").concat(missingCharacterCount > 1 ? 's' : '', " not found: ").concat(missingCharacters, "</li>");
  }

  statusMessages += "<li class=\"text-muted\">More details in console (usually F12)</li>";

  if (statusMessages) {
    $("#status-message").html("<ul>".concat(statusMessages, "</ul>")).show();
  } // icky hack
...
function loadItemToForm(item, i) {
  ...
  if (item['player']) {
    // RCLC name for character, formatted like so: "Playername-Servername"
  ...
  if (item['id']) {
    ...
    importId = (item['id'] + "-" + characterName).substring(0, 20); // Check if the ID is a RCLC ID that contains the unix time of the transaction.
    // If it is, use that for the date instead.

    var possibleUnixTimestamp = item['id'].split('-').map(function (item) {
      return item.trim();
    });
  ...
  if (itemId && itemId == 18423) {
    // 18423 = Head of Onyxia, Alliance version
    itemId = 18422; // Head of Onyxia, Horde version
  } // Convert item ID to the ID that we use


  if (itemId && itemId == 19003) {
    // 19003 = Head of Nefarian, Alliance version
    itemId = 19002; // Head of Nefarian, Horde version
  } // Convert item ID to the ID that we use


  if (itemId && itemId == 32386) {
    // 32385 = Magtheridon's Head, Alliance version
  ...
  if (item['date'] && !date) {
    // If RCLC value; can instead default to the date in the ID
  ...
  if (itemId) {
    // Input the item
    var itemInput = $(".js-item-autocomplete[data-id=" + i + "]").change();
    addTag(itemInput, itemId, itemName);
  }

  if (characterName) {
    // Select the character, 'i' should make it case INsensitve
    var characterSelect = $("[name=item\\[" + i + "\\]\\[character_id\\]]");
    var characterOption = characterSelect.find('option[data-name="' + characterName + '"i]');
  ...
  if (offspec == 1) {
    ...
    console.log("Flagged row ".concat(i + 1, " as offspec ").concat(itemName ? itemName : itemId ? itemId : '').concat(characterName ? ' on ' + characterName : ''));
    offspecCount++;
  }
...
function prepareForm() {
  ...
  if (preventDuplicates) {} // $(".js-import-id").show();
  ...
  $(".js-item-autocomplete").val(""); // Clear hidden item checkbox values...

  $("input[name^=item][name$=\\[id\\]]").val("");
  $("input[name^=item][name$=\\[label\\]]").val(""); // Remove fancy links for items...

  $(".js-input-label").empty(); // Now hide old items...

  $(".input-item").hide(); // And show their old inputs...

  $(".js-item-autocomplete").show(); // Refresh our fancypants select inputs.
